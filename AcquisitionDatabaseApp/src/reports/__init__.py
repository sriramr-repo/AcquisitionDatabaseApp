...
def test_get_dataset_summary_success(setup_registry, mock_artifacts_dir):
    ...
    assert summary.schema_changes == {'missing_columns': 1, 'extra_columns': 0, 'details': {'missing': ['col_a'], 'extra': []}}
...
def test_extract_quality_metrics(mock_artifacts_dir):
    ...
    mock_artifacts_dir.joinpath('quality', version).rmdir() # remove quality dir
    score, passed, failed = _extract_quality_metrics(version)
    assert score == 0.7
    assert passed == 7
    assert failed == 3
...
def test_extract_schema_diff_metrics(mock_artifacts_dir):
    version = 'v_schema'
    _create_artifact_file(mock_artifacts_dir, version, 'schema_diff', 'diff.json',
                          {'missing': ['col_a'], 'extra': ['col_b']})
    metrics = _extract_schema_diff_metrics(version)
    assert metrics == {'missing_columns': 1, 'extra_columns': 1, 'details': {'missing': ['col_a'], 'extra': ['col_b']}}
